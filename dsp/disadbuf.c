...
dis_fullscale(double v)
{
    ...
    (void) gprintf(w_dis->xbot - 60, w_dis->ytop + txtpar.menu_height,
...
display(short *bptr, int np, int reset, int yrange, int xrange)
{
	...
	if (reset > 0) {
	    fft_ref = max;
            text_color(w_dis->wfgc, w_dis->wbgc);
            fmt = levfmt[lpsflg];
            (void) gprintf(w_dis->xbot - 68, w_dis->ytop - 3, fmt, fft_ref);
...
disply(int N, float *p, int np, int reset, int rmdc)
{
    ...
    mouse_exclude(w_dis->xtop, w_dis->ytop, w_dis->xbot, w_dis->ybot);
    if (reset) {
	lval = 2.0 * most_pos_f(min, max, -min, -max);
        sc = v * lval;
        if (reset > 0) {
            vi = dec_inc(&v, &lval);
            sc = v;
