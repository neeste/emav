...
reset_io(int bs)
{
    ...
    if (iodev == WA_DEV)
        bs = wa_reset_io(bs);
#endif // WIN32
    bio_.bank_b = (int32_t *) calloc(bs * ndc[0], sizeof(int32_t));
...
stim_fix()
{
	...
	for (b = 0; b < nsbc; b++) {
		...
		for (i = j = 0; i < nsstm; i++, j += dj) {
		    s = (int32_t) (ob[j] / sc);
		    sl[j] = (int32_t) limit(-mxsmp[1], s, mxsmp[1]);
		}
	    }
	}
    }
...
CARDINFO
sio_get_cardinfo(int i)
{
#ifdef WIN32
    return (wa_get_cardinfo(i));
#else // WIN32
    CARDINFO ci;
    return (ci);
#endif // WIN32
...
void
sio_set_cardinfo(CARDINFO ci, int i)
{
#ifdef WIN32
    wa_set_cardinfo(ci, i);
#endif // WIN32
}
