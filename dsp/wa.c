...
static int32_t *lbanki[NBNK];
static int32_t *lbanko[NBNK];
static short *sbanki[NBNK];
static short *sbanko[NBNK];
static uint32_t rate = 44100;
static uint32_t goodSR[SRLSTSZ];
static uint32_t SRlist[SRLSTSZ] = {
...
wa_set_rate(double r)
{
    ...
    rt = (uint32_t) wa_adjust_rate(r);
    if ((rt > 0) && (rt != (uint32_t) rate)) {
	rate = rt;
        if (wa_dev_opened) {
	    close_io();
	    open_io();
