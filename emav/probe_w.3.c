...
/* characteristic impedance of tube */
static double
z_tube(float *z)
{
...
z_surge(float *zl, int na)
{
    ...
    z_chr(zc);
    z0 = zc[0];
    R = (unsigned long)((double *) calloc((size_t)(na * 2), sizeof(double)));
    c_ones(R, na);
    fd_window(R, na);
    w0 = mean_real(R, na);
    for (k = 0; k < nk; k++) {
...
thev_load(char *cfn, char *sfn)
{
    ...
    line[0] = 0;
    if (ths_rate != rate) {
        sprintf(line, "rate = %ld %ld", ths_rate, rate);
