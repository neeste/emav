...
static void 
reflect(float *R, float *zl, double z0, int n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        rdi = zl[ii];
        rdm = rdr * rdr + rdi * rdi;
        R[ir] = (float) ((rnr * rdr + rni * rdi) / rdm);
        R[ii] = (float) ((rni * rdr - rnr * rdi) / rdm);
    }
}

static void
z_surge(float *zo, int na)
{
    double z0, w0, wr;
    float zc[2], *R;
    int k, nk;
...
thev_init()
{
    ...
    qq1 = (float *) calloc((size_t)(MAX_NF * 2 * 2), sizeof(float));
    if (qq1 == NULL) {
...
//    ppv = Sen.MP * Sen.DA[c] / Sen.AD;
    ppv = 2 * Sen.DA[c] / (Sen.AD * Sen.MP * s);  // for consistency in 3.21 ???
    for (i = 0; i < n; i++) {
        ir = 2 * i;
        ii = 2 * i + 1;
        p0.x = st[ir];
...
thev_source(char *fn)
{
	...
	for (j = 0; j < numsrc; j++) {
		...
		nftf = fft_sig(buf, cal_sets);
                ...
                divide_by_stim(pc[i], nthf, j, swp1set);
...
static void
ldimp(float *zl, float *zs, float *ps, float *pc, int na, double sg)
{
...
static double
surge_gain(float *zl, float *zs, float *ps, float *pc, int na)
{
    double sg = 1;
    float zc[2];
    int i, nk;
...
thev_cmp_zl()
{
    ...
    double   z0r, z0i, zlr, zli, zlm, psr, zsr, zsi, sg;
    float    zlrmin, zo[2], *pc;
    int     i, ii, ir;
    static float eps = 1e-9F;
...
thev_load(char *cfn, char *sfn)
{
	...
	save_stim_fft(1);
        ...
        divide_by_stim(pl, nthf, j, swp1set);
	...
	stat_msg("Error writing load impedance file.");
    } else {
...
#ifdef WIN32
    char cmd[80];
    static char *pfn = "C:/Program Files/BTNRH/Stepper/StepperSigpro.exe";

    if (_access(pfn, 0) == 0) {
        if (decide(1, 1, "Launch Stepper?")) {
            sprintf(cmd, "\"%s\"", pfn);
            system(cmd);
            return(1);
        }
