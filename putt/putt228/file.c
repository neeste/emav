...
check_hearing_file(char *fn, int flag)
{
    ...
    fpt = fopen(fn, "rt");
    if (fpt == NULL) {
    	if(flag) decide(0, 2, "Can't open file",
	    "Press any key or mouse to continue");
	return(fpt);
    }
    fgets(line, MAXLINE, fpt);
...
void rd_hearing_file(void)
{
    ...
    fpt = check_hearing_file(o_file_name, 1);
    if (fpt == NULL)
    return;
    init_wind();
	...
	if (line[0] == ';') {
		...
		break;
	    case 5:		/* LastName */
		strncpy(patient.Lastname, start, 13);
		break;
	    case 6:		/* FirstName */
		strncpy(patient.Firstname, start, 12);
		break;
	    case 7:		/* ID */
		strncpy(patient.ID, start, 12);
		break;
	    case 8:		/* Ear */
		k = toupper(tokstr[0]);
		if (k == 'L')
    ...
    while(*otherfn) {
        fpt = check_hearing_file(otherfn, 1);
        if (fpt != NULL) {
