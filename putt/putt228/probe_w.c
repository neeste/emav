...
extern int lpsflg, fft_flag, prob_cal_cnt;
...
static char fnf[FNSZ] = "probe.fnf";
static int icav = 0;
static int nftf = 0;
static int nthf = 0;
static int nthc = 0;
...
static double
len_z1(double mxlc, double f)
{
    double df, lc, fz, f0;
    float *zm, zt[2];
    int i, j, i1, i2;
    static double a = 0.014, b = 8500;

    df = (double) rate / buflen;
    f0 = b / (mxlc - a);
    i1 = (int) (0.5 * f0 / df + 0.5);
    i2 = (int) (2.5 * f0 / df + 0.5);
    zm = qq1;
    lc = a + b / f;
    for (j = 0; j < 8; j++) {
        for (i = 1; i < nthf; i++) {
            z_cav(zt, i * df, lc, diacav, tmpcav);
            zm[i] = (float) _hypot(zt[0],zt[1]);
        }
        fz = pkmn(i1, i2, 1, zm) * df;
        lc = a + (lc - a) * fz / f;
    }
    return (lc);
}
...
static double
len_pk(double mxlc, double f)
{
    double df, lc, fp, f0;
    float *zm, zt[2];
    int i, j, i1, i2;
    static double b = 16850;
...
thev_source(char *fn1, char *fn2)
{
	...
	for (i = 0; i < nthc; i++) {
	    ...
	    modulesqr();
	    if (f0 > 1000) {
                fz1[i] = (float) pick_min(0.5 * f0, 2.5 * f0);
                fpk[i] = (float) pick_max(fz1[i], 3 * fz1[i]);
                lcv[i] = (float) len_z1(probe.maxlen, fz1[i]);
            } else {
                fpk[i] = (float) pick_max(f0, 3 * f0);
                fz1[i] = (float) pick_min(0.25 * fpk[i], 0.75 * fpk[i]);
                lcv[i] = (float) len_pk(probe.maxlen, fpk[i]);
            }
	}
	dfgc = scrn_c[C_WAVE];
	cal_close();
...
thev_load(char *fn)
{
    ...
    sets = nsets * 2;
    reps = sets * swp1set;
    cal_ppc = 1 / (Sen.MP * Sen.AD * reps);                 /* Pa/cnt */
...
thev_adjust(double f, double spl, double *sil, double *fpl, double *tml)
{
    ...
    *fpl = spl + fpladj[i];	// FPL adjustment
    *tml = spl + tmladj[i];	// SPL_TM adjustment
}
