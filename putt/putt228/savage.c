...
MENUITEM h_option_3[] = {
    ...
    {"&Ramp Time (ms) :", (char *) &ramp_time, FLOAT, 6, 2, 1, chk_ramp_tone},
...
MENUITEM h_option[] = {
    ...
    {"Si&muluate Observer =", (char *) &sim_obs, TOGGLE, 3, 0, 1, NULL},
    {NULL, NULL, NONE, 0, 0, 1, NULL}
};
...
MENUITEM t_option[] = {
    ...
    {"Ramp &Time (ms) :", (char *) &ramp_time, FLOAT, 6, 0, 1, chk_ramp_tone},
...
MENUITEM sen_opt[] = {
    {"&AD (cnt/V) :", (char *) &Sen.AD, FLOAT, 6, 1, 1, NULL},
...
int chk_ramp_tone(void)
{
    ...
    r = (int) (ramp_time * (float) tone.rate / 1000.0 + 0.5);
    if(r < 64) {
    	i = 0;
    } else if (r >= tone.size) {
	i = tone.size;
    ...
    tone.ramp = i;
    ramp_time = (float) (i * 1000.0 / tone.rate);
    return(0);
}
...
hr_opt(char *no_use)
{
    ...
    ramp_time = (float) (tone.ramp * 1000.0 / tone.rate);
    chk_ramp_tone();
    thresh_hf.at = hear.hfthr;
    ret = simple_submenu(menu_x(), menu_y(), h_option);
    hear.hfthr = thresh_hf.at;
    return (ret);
}
...
void dsp_vfs(void)
{
    ...
    if ((Sen.AD == 0) && ad_vfs[0] != 0) {
	Sen.AD = (float)((pow(2, 23) - 1) / fabs(ad_vfs[0]));
    }
    if ((Sen.DA[0] == 0) && da_vfs[0] != 0) {
        Sen.DA[0] = (float)((pow(2, 15) - 1) / fabs(da_vfs[0]));
    }
    if ((Sen.DA[1] == 0) && da_vfs[1] != 0) {
        Sen.DA[1] = (float)((pow(2, 15) - 1) / fabs(da_vfs[1]));
    }
}
...
getstim(char *stimfn, int np)
{
    ...
    while (!valid && strlen(stimfn) > 0 && !pgm_done()) {
	    ...
	    valid = (rdils(stimfn, outbuf, (long) buflen, &rrate, &npts) > 0);
	}
	if (!valid) {
	    gr_beep();
	    k = open_text_w(&i);
