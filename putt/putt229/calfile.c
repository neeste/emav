...
static int
mat_rd_hdr(int fd, char *typ, long * hdr, char *nam)
{
    int nc = 0;
...
mat_read(int tn)
{
    ...
    if (cal_vpc > 1 || cal_vpc < 1e-12) /* fix files before 29-Jul-96 */
        cal_vpc = 9.65e-6;
    Sen.AD = (float) (1 / cal_vpc);		/* cnt/V  */
    Sen.MP = (float) (cal_vpc / cal_ppc);	/* V/Pa   */
}
...
{
    strncpy(hpt, tfp->header.pattern, 12);
...
static void
scaleback(int tokmax, long maxval, float *out)
{
    int     i;
    float   scale;
...
tok_read(int tn)
{
    ...
    if ((c->adsen > 1e3) && (c->adsen < 1e9) && 
        ...
        Sen.MP = ((float) c->maxval / c->tokmax) /
            (c->reps * c->scale * Sen.AD);
    }
...
static void
mat_wr1_l(int fd, char *nam, long p)
{
...
mat_store(int c, float *p)
{
    ...
    sprintf(nam, "car%d", tcount); mat_wr_f(mfd, nam, car, 6);
//    sprintf(nam, "stm%d", tcount); mat_wr_a(mfd, nam, ilstim);
    sprintf(nam, "stm%d", tcount); mat_wr_s(mfd, nam, stimulus, buflen);
    sprintf(nam, "tim%d", tcount); mat_wr1_l(mfd, nam, (long) time(NULL));
}
